...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if new_ant.can_contain(curr_ant):
                place.ant = new_ant
                new_ant.store_ant(curr_ant)
            elif curr_ant.can_contain(new_ant):
                curr_ant.store_ant(new_ant)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while (not curr_place.is_hive):
            if (len(curr_place.bees) > 0 and self.lower_bound <= tracker <= self.upper_bound):
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if (isinstance(other, ContainerAnt) or self.ant_contained != None):
            return False
        else:
            return True
        # END Problem 8a
    ...
    def action(self, gamestate):
        ...
        while (curr_place != None):
            if (curr_place.ant != None):
                curr_place.ant.double()
                if (isinstance(curr_place.ant, ContainerAnt)):
                    if (curr_place.ant.ant_contained != None):
                        curr_place.ant.ant_contained.double()
    ...
    def throw_at(self, target):
        ...
        def slowed(action):
            ...
            def slow_action(*args):
                if target.slow_timer <= 0:
                    action(target, args[0])
                elif args[0].time % 2 == 0:
                    action(target, args[0])
        ...
        if target is not None:
            target.action = slowed(Bee.action)
